...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let cur_phrase, lin_idx, guessing, last_key_is_del, last_key_is_create, formattedDateX, puzScore,
      BOXROWS, BOXCOLS, num_arr, str_arr, guess_arr, last_lins, first_lins, underline1, underline2;
  let guessed_letts = '';
  let puzzle_solved, info_showing, clueLog = false;
  ...
  function createKeyboard() {
    keyboard.innerHTML = '';
    const rows = [
      { keys: 'QWERTYUIOP', clue: false, guess: false, backspace: false },
      { keys: 'ASDFGHJKL', clue: false, guess: false, backspace: false },
      { keys: 'ZXCVBNM', clue: false, guess: false, backspace: false },
      { keys: '', clue: true, guess: true, backspace: true}
    ];
    ...
    rows.forEach(row => {
      ...
      if (row.clue) {
        ...
        clueButton.classList.add('clue-shift-key-btn');
        clueButton.textContent = 'Clue';
        clueButton.addEventListener('click', () => {
          handleKeyPress(' ');
        });
        rowDiv.appendChild(clueButton);
      }
