...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const solvedToBlur = document.querySelectorAll("body > :not(#solved-popup):not(#share-button-pop):not(#logo):not(#date-display):not(#container)");
  const infoToBlur = document.querySelectorAll("body > :not(#info-popup):not(#pause-popup):not(solved-popup");
  ...
  const pauseToBlur = document.querySelectorAll("body > :not(#pause-popup):not(#info-popup):not(solved-popup):not(#logo):not(#date-display):not(#info-button)");
  
  let cur_phrase, lin_idx, guessing, last_key_is_del, last_key_is_create, formattedDateX, puzScore, pause_showing,
  ...
  function cycleCorrect(correct) {
    if (correct) {
      if (cur_phrase != last_lins.length - 1) {
        ...
        solvedScore.textContent = Math.round(puzScore * 10) / 10;
        solvedBlur(true);
      }
    }
  }
  ...
  function handleTimer() {
    ...
    intervalId = setInterval(() => {
      if (!puzzle_solved && !pause_showing && !info_showing) {
          puzScore -= 0.1;
          updateScoreDisplay(puzScore);
      }
    }, 1000);
  ...
  document.addEventListener('keydown', (event) => {
    if ((puzzle_solved || info_showing || pause_showing) && event.key !== 'Escape') {
      return;
    }
